	...
	case CCamera::FOLLOW_TARGET_AT_DISTANCE_CHASE_PLANE:
		{
			...
			if ( this->eye.x < targetProps.position.x )	{ this->eye.x += changeThisStep.x; }
			else if ( this->eye.x > targetProps.position.x )	{ this->eye.x -= changeThisStep.x; }

			if ( this->eye.y < targetProps.position.y )	{ this->eye.y += changeThisStep.y; }
			else if ( this->eye.y > targetProps.position.y )	{ this->eye.y -= changeThisStep.y; }

			if ( this->eye.z < targetProps.position.z )	{ this->eye.z += changeThisStep.z; }
			else if ( this->eye.z > targetProps.position.z )	{ this->eye.z -= changeThisStep.z; }
		}

	...
	for ( std::vector< IGameObjectPhysics* >::iterator itGO = vecGameObjects.begin(); itGO != vecGameObjects.end(); itGO++ )
	{
		...
		{	// yup, so do the magic
			...
			{	// Update it
				...
				if (curPhysProps.velocity.x > maxVelocity)
					curPhysProps.velocity.x = maxVelocity;
				if (curPhysProps.velocity.y > maxVelocity)
					curPhysProps.velocity.y = maxVelocity;
				if (curPhysProps.velocity.z > maxVelocity)
					curPhysProps.velocity.z = maxVelocity;

...
void StoreAABB(unsigned int player_index, unsigned int player_Cindex, CAABB* pAnimalAABB)
{
	g_mapCAABB[player_index]->map_pchildAABB[player_Cindex]->vec_pMazeAABB.push_back(pAnimalAABB);
	//g_mapCAABB[player_index]->map_pchildAABB[player_Cindex]->map_pchildAABB[1] = pAnimalAABB;
	if (g_mapCAABB[player_index]->map_pchildAABB[player_Cindex]->bHasTriangleInIt == false){
		g_mapCAABB[player_index]->map_pchildAABB[player_Cindex]->bHasTriangleInIt = true;
		if (g_mapCAABB[player_index]->bHasTriangleInIt == false)
			g_mapCAABB[player_index]->bHasTriangleInIt = true;
	}
}
...
void GenerateAABBWorld()
{
	...
	for (int animalIndex = 0; animalIndex < g_vecMazeIDs.size(); animalIndex++)
	{
		std::string plyFileName;
		std::vector<CMeshDesc> mazeDesc;
		g_pFactoryMediator->getRenderingPropertiesByID(g_vecMazeIDs[animalIndex], mazeDesc);
		plyFileName = mazeDesc[0].modelName;
		//g_pFactoryMediator->GetPlyFileByID(g_vecMazeIDs[animalIndex], plyFileName);
		CPlyFile5nt* pEnvironment = ::g_pModelLoader->getOriginalModelInfo(plyFileName);
		CVector3f levelPosition; //adjusts for the map moving along the x, y, or z axis
		CPhysicalProp mazeProp;
		::g_pFactoryMediator->getPhysicalPropertiesByID(g_vecMazeIDs[animalIndex], mazeProp);
		levelPosition = mazeProp.position;
		//::g_pFactoryMediator->GetPositionByID(g_vecMazeIDs[animalIndex], levelPosition);
		int anIndex = 0;

		CAABB* pAnimalAABB = new CAABB();
		pAnimalAABB->cubeSize = 2.0f;
		pAnimalAABB->minXYZ.x = levelPosition.x;
		pAnimalAABB->minXYZ.y = levelPosition.y;
		pAnimalAABB->minXYZ.z = levelPosition.z;
		pAnimalAABB->m_index = anIndex;

		int indexIndent = 10;
		float min_xPosC = ((int)pAnimalAABB->minXYZ.x - (int)pAnimalAABB->minXYZ.x % indexIndent) * 0.1f;
		float min_yPosC = ((int)pAnimalAABB->minXYZ.y - (int)pAnimalAABB->minXYZ.y % indexIndent) * 0.1f;
		float min_zPosC = ((int)pAnimalAABB->minXYZ.z - (int)pAnimalAABB->minXYZ.z % indexIndent) * 0.1f;
		float min_xPos = min_xPosC * 0.1f;
		float min_yPos = min_yPosC * 0.1f;
		float min_zPos = min_zPosC * 0.1f;
		...
		float max_xPosC = (((int)pAnimalAABB->minXYZ.x + (int)pAnimalAABB->cubeSize) - ((int)pAnimalAABB->minXYZ.x + (int)pAnimalAABB->cubeSize) % indexIndent) * 0.1f;
		float max_yPosC = (((int)pAnimalAABB->minXYZ.y + (int)pAnimalAABB->cubeSize) - ((int)pAnimalAABB->minXYZ.y + (int)pAnimalAABB->cubeSize) % indexIndent) * 0.1f;
		float max_zPosC = (((int)pAnimalAABB->minXYZ.z + (int)pAnimalAABB->cubeSize) - ((int)pAnimalAABB->minXYZ.z + (int)pAnimalAABB->cubeSize) % indexIndent) * 0.1f;
		float max_xPos = max_xPosC * 0.1f;
		float max_yPos = max_yPosC * 0.1f;
		float max_zPos = max_zPosC * 0.1f;
		...
		StoreAABB(min_player_index, min_player_Cindex, pAnimalAABB);


		if (min_player_Cindex != max_player_Cindex){
			StoreAABB(max_player_index, max_player_Cindex, pAnimalAABB);
			unsigned int otherCorners;
			//only x is max
			otherCorners = genIndex((int)max_xPosC, (int)min_yPosC, (int)min_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)max_xPos * 10, (int)min_yPos * 10, (int)min_zPos * 10), otherCorners, pAnimalAABB);
			//only y is max
			otherCorners = genIndex((int)min_xPosC, (int)max_yPosC, (int)min_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)min_xPos * 10, (int)max_yPos * 10, (int)min_zPos * 10), otherCorners, pAnimalAABB);
			//only z is max
			otherCorners = genIndex((int)min_xPosC, (int)min_yPosC, (int)max_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)min_xPos * 10, (int)min_yPos * 10, (int)max_zPos * 10), otherCorners, pAnimalAABB);
			// x and y are max
			otherCorners = genIndex((int)max_xPosC, (int)max_yPosC, (int)min_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)max_xPos * 10, (int)max_yPos * 10, (int)min_zPos * 10), otherCorners, pAnimalAABB);
			// x and z are max
			otherCorners = genIndex((int)max_xPosC, (int)min_yPosC, (int)max_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)max_xPos * 10, (int)min_yPos * 10, (int)max_zPos * 10), otherCorners, pAnimalAABB);
			// y and z are max
			otherCorners = genIndex((int)min_xPosC, (int)max_yPosC, (int)max_zPosC);
			if (otherCorners != min_player_Cindex && otherCorners != max_player_Cindex)
				StoreAABB(genIndex((int)min_xPos * 10, (int)max_yPos * 10, (int)max_zPos * 10), otherCorners, pAnimalAABB);
		}
